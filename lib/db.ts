/**
 * Turso / libSQL client.
 * Schema bootstrap is fire-and-forget — it NEVER blocks page render.
 * Pages wrap their DB calls in try/catch and render empty states on failure.
 */

// Suppress @libsql/client migration-job poller (GET /v1/jobs → 400)
...
const TABLE_DDL: Record<string, string[]> = {
  courses: [
    `CREATE TABLE IF NOT EXISTS courses (id INTEGER PRIMARY KEY AUTOINCREMENT, canvas_id TEXT NOT NULL, name TEXT NOT NULL, course_code TEXT, term TEXT, accent_color TEXT, last_seen_at TEXT NOT NULL DEFAULT (datetime('now')), created_at TEXT NOT NULL DEFAULT (datetime('now')))`,
  ...
  sync_log: [
    `CREATE TABLE IF NOT EXISTS sync_log (id INTEGER PRIMARY KEY AUTOINCREMENT, status TEXT NOT NULL, tasks_upserted INTEGER NOT NULL DEFAULT 0, courses_processed INTEGER NOT NULL DEFAULT 0, error_message TEXT, duration_ms INTEGER, started_at TEXT NOT NULL DEFAULT (datetime('now')), finished_at TEXT)`,
  ],
  push_subscriptions: [
    `CREATE TABLE IF NOT EXISTS push_subscriptions (id INTEGER PRIMARY KEY AUTOINCREMENT, endpoint TEXT NOT NULL, p256dh_key TEXT NOT NULL, auth_key TEXT NOT NULL, user_agent TEXT, created_at TEXT NOT NULL DEFAULT (datetime('now')))`,
  ...
  user_settings: [
    `CREATE TABLE IF NOT EXISTS user_settings (id INTEGER PRIMARY KEY, ical_url TEXT, ical_label TEXT, created_at TEXT NOT NULL DEFAULT (datetime('now')), updated_at TEXT NOT NULL DEFAULT (datetime('now')))`,
...
async function httpExec(baseHttps: string, token: string, sql: string): Promise<Array<Record<string, unknown>>> {
  const res = await fetch(`${baseHttps}/v2/pipeline`, {
    method: "POST",
    headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
    body: JSON.stringify({ requests: [{ type: "execute", stmt: { sql } }, { type: "close" }] }),
  });
  if (!res.ok) throw new Error(`Turso ${res.status}: ${sql.slice(0, 60)}`);
...
async function runBootstrap(): Promise<void> {
  ...
  // One query — get all existing tables
  const rows    = await httpExec(base, token, `SELECT name FROM sqlite_master WHERE type='table'`);
  ...
  for (const table of missing) {
    for (const ddl of TABLE_DDL[table] ?? []) {
      await httpExec(base, token, ddl);
    }
  }

  // Column migrations — one PRAGMA per table
  for (const [table, migs] of Object.entries(COLUMN_MIGRATIONS)) {
    if (!existing.has(table)) continue;
    const cols = await httpExec(base, token, `SELECT name FROM pragma_table_info('${table}')`);
    const colSet = new Set(cols.map((r) => String(r.name ?? "")));
    for (const m of migs) {
      if (!colSet.has(m.column)) await httpExec(base, token, m.ddl);
    }
  }
}
